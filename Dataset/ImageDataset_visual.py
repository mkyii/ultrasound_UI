...
class VisualDataset(Dataset):  # PyTorch Dataset 상속
    def __init__(self, root_path, mask_path, label_path, mode='train',
        ...
        for _, row in self.labels_df.iterrows():
            image_file = os.path.join(self.root_path, row['folder_name'], row['file_name'] + '.png')
            mask_file  = os.path.join(self.mask_path, row['folder_name'], row['file_name'] + '.png')
    ...
    def __getitem__(self, idx):
        ...
        if sample['mask'] and os.path.exists(sample['mask']):
            ...
            mask_tensor = self.mask_transform(mask).long()
        else:
